  ...
  return Array(9)
    ...
    .map((motionValue, index) => {
      return {
        ...
        debuffs,
        transforms,
        stats: [motionValue === 0.8365 ? 'normal' : 'charge', 'pyro'],
      };
    });
};

export const afterlifeAction = ({
  weapon,
  artifacts,
  buffs = [],
  debuffs,
...
export const print = () => {
  withContext({
    ...
    run: () => {
      const vapeHits = hits(
        [afterlife, geoRes, tom, homNature],
        [lapidus],
        [icdPyroVape]
      );
      ...
      const vvVapeHits = hits(
        [afterlife, tom, pyroRes, sucroseSwirl, sucroseC6],
        [vvShred],
        [icdPyroVape]
      );
      ...
      console.log(
        `Homa r1: ${damageDps(stats(char, homa, cr_artifacts), vapeHits, 9)}`
      );
      ...
      console.log(
        `Homa r1 one charge CRIT: ${damageDps(
          stats(char, homa, cr_artifacts.concat({ critRate: 1 })),
          vapeHits.slice(1, 2)
        )}`
      );
      ...
      console.log(
        `Homa r1: ${damageDps(stats(char, homa, cr_artifacts), vvVapeHits, 9)}`
      );
