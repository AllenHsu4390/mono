...
char.traits = [
  ({ elemDmg }, { stats }) => {
    return {
      elemDmg: elemDmg + (stats.includes('geo') ? 0.288 : 0),
    };
  },
];
char.lvl = 90;
...
export const hits = (
  ...
  return Array(Math.floor(Math.min(duration, 30) / 2))
    .fill(BLOSSOM)
...
export const print = () => {
  withContext({
    ...
    run: () => {
      ...
      console.log(
        `Cinnabar Blossom Damage: ${damageDps(
          stats(char, cinnabar, artifacts),
          hits(buffs, debuffs, undefined, undefined, undefined, 10),
          10,
          0,
          'def'
        )}`
      ...
      console.log(
        `Festering Blossom Damage: ${damageDps(
      ...
      console.log(
        `Cinnabar Blossom Damage: ${damageDps(
          stats(char, cinnabar, artifacts),
          hits([], debuffs, undefined, undefined, undefined, 10),
          10,
          0,
          'def'
        )}`
      ...
      console.log('');
      console.log(
        `Cinnabar Blossom Damage 1 Hit CRIT: ${damageDps(
          stats(char, cinnabar, artifacts.concat({ critRate: 1 })),
          hits(buffs, debuffs).slice(0, 1),
          undefined,
          undefined,
          'def'
        )}`
      ...
      console.log(
        `HOD Blossom Damage 1 Hit CRIT: ${damageDps(
      ...
      console.log(
        `Festering Blossom Damage 1 Hit CRIT: ${damageDps(
      ...
      console.log('');
      console.log(
        `Cinnabar Burst: ${damageDps(
          stats(char, cinnabar, artifacts.concat({ critRate: 1 })),
          burst([geoRes, ...buffs], debuffs).slice(0, 1),
          undefined,
          undefined
        )}`
      );
