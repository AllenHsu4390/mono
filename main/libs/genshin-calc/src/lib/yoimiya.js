...
export const hits = (traits, debuffs, amps, transforms, stats = []) => {
  return Array(3)
    .fill([0.6359, 0.6359, 1.2199, 1.5859, 0.8282, 0.8282, 1.8887])
    .flat()
    ...
    .concat(
      [1.91, ...Array(5).fill(1.83)].map((motionValue, index) => {
        return {
          stats: ['pyro', 'burst'],
          traits,
          amplifiers: [crit],
...
export const print = () => {
  withContext({
    enemy,
    run: () => {
      const vapeOverloadPyro = hits(
        pyroTeam,
        undefined,
        [firedance, icdPyroVape],
        [icdOverloaded]
      );
      const vapePyro = hits(pyroTeam, [lapidus], [firedance, icdPyroVape], []);
      const overloadPyro = hits(
        pyroTeam,
        [lapidus],
        [firedance],
        [icdOverloaded]
      ...
      const overloadGeo = hits(
        geoTeam,
        [lapidus],
        [firedance],
        [icdOverloaded]
      );
      const vapeOverload = hits(
        mixTeam,
        [lapidus],
        [firedance, icdPyroVape],
        [icdOverloaded]
      ...
      const vvVape = hits(
        [tom, pyroRes, sucroseSwirl, sucroseC6],
        [vvShred],
        [icdPyroVape],
        []
      );
      ...
      console.log(
        `Rust r5 CR circ: ${damageDps(
          stats(yoimiya, rustR5, artifacts),
          vapeOverloadPyro,
          10
        )}`
      );
      ...
      console.log(
        `Rust r5 CR circ Last Hit CRIT: ${damageDps(
          stats(yoimiya, rustR5, artifacts.concat({ critRate: 1 })),
          vapeOverloadPyro.slice(6, 7)
        )}`
      );

      console.log(
        `Skyward Harp CD circ: ${damageDps(
          stats(yoimiya, skywardHarpR1, [
            flower_4,
            feather_4,
            sand_4,
            goblet_4,
      ...
      console.log(
        `Skyward Harp CD circ Last Hit CRIT: ${damageDps(
          stats(
            yoimiya,
            skywardHarpR1,
            [flower_4, feather_4, sand_4, goblet_4, cd_circ].concat({
              critRate: 1,
            })
          ),
          vapeOverloadPyro.slice(6, 7)
      ...
      console.log(
        `Thundering Pulse CR circ: ${damageDps(
          stats(yoimiya, thunderingPulseR1, artifacts),
          vapeOverloadPyro,
          10
        )}`
      );
      console.log(
        `Thundering Pulse CR circ Last Hit CRIT: ${damageDps(
          stats(yoimiya, thunderingPulseR1, artifacts.concat({ critRate: 1 })),
          vapeOverloadPyro.slice(6, 7)
        )}`
      );
      console.log('');
